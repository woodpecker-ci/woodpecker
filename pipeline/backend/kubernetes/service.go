...
func startHeadlessService(ctx context.Context, engine *kube, namespace, taskUUID string) (*kube_core_v1.Service, error) {
	...
	createdSvc, err := engine.client.CoreV1().Services(namespace).Create(ctx, svc, kube_meta_v1.CreateOptions{})
	if err != nil {
		if errors.IsAlreadyExists(err) {
			// Service already exists (recovery scenario), continue without error
			log.Trace().Str("name", svc.Name).Msg("headless service already exists, reusing")
			return svc, nil
		}
		return nil, err
	}
