...
func startPod(ctx context.Context, engine *kube, step *types.Step, options BackendOptions) (*v1.Pod, error) {
	...
	return engine.client.CoreV1().Pods(engineConfig.K8sNamespace(step.OrgID)).Create(ctx, pod, meta_v1.CreateOptions{})
}
...
func stopPod(ctx context.Context, engine *kube, step *types.Step, deleteOpts meta_v1.DeleteOptions) error {
	...
	err = engine.client.CoreV1().Pods(engine.config.K8sNamespace(step.OrgID)).Delete(ctx, podName, deleteOpts)
