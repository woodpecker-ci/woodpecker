...
func AdhocSSHElevated(ctx context.Context, client *goph.Client, cmd string, args []string, taskUUID string, stepUUID string) error {
	var out string
	// first figure out if we have 'sudo' or 'doas', so we can have the right privilege escalation mechanism
	{
		cmd := "/bin/sh"
		args := []string{"-c", `'if command -v sudo >/dev/null 2>&1 ; then printf "%s" "sudo" ; elif command -v doas >/dev/null 2>&1 ; then printf "%s" "doas" ; else exit 0 ; fi'`}
		sshCmd, err := client.CommandContext(ctx, cmd, args...)
		if err != nil {
			return err
		}
	...
	switch out {
	case "sudo":
		command = append([]string{"sudo", cmd}, args...)
	case "doas":
		command = append([]string{"doas", cmd}, args...)
	case "":
	...
	// execute command
	return AdhocSSH(ctx, client, command[0], command[1:], taskUUID, stepUUID)
}
