...
func (e *libvirt) StartStep(ctx context.Context, step *backend_types.Step, taskUUID string) (ret_err error) {
	...
	case "freebsd":
		// create dir
		{
			err := AdhocSSHElevated(ctx, client, "mkdir", []string{"-p", step.WorkspaceBase}, taskUUID, step.UUID)
			if err != nil {
				return err
			}
		}
		...
		// mount
		{
			err := AdhocSSHElevated(ctx, client, "mount", []string{fmt.Sprintf("/dev/gptid/%s", uuid), step.WorkspaceBase}, taskUUID, step.UUID)
			if err != nil {
				return err
			}
		}
	case "darwin":
		...
		// mount
		{
			err := AdhocSSHElevated(ctx, client, "diskutil", []string{"mount", "-mountPoint", step.WorkspaceBase, uuid}, taskUUID, step.UUID)
			if err != nil {
				return err
			}
		}
	default:
		...
		// mount
		{
			...
			err := AdhocSSHElevated(ctx, client, "mount", []string{"-o", mountSource, step.WorkspaceBase}, taskUUID, step.UUID)
			if err != nil {
				return err
			}
		}
...
func (e *libvirt) DestroyStep(ctx context.Context, step *backend_types.Step, taskUUID string) error {
	...
	defer func() {
		...
		if !keep_tmp {
			...
			nvram := filepath.Join(libvirtImgDir, fmt.Sprintf(nvramImagePattern, taskUUID, step.UUID))
			_, statErr := os.Stat(nvram)
			if statErr == nil {
				log.Debug().Msgf("Deleting nvram image: %s", nvram)
				err := os.Remove(nvram)
				if err != nil {
					log.Debug().Msgf("Error while unlocking step mutex: %s", err.Error())
				}
...
func (e *libvirt) DestroyWorkflow(ctx context.Context, conf *backend_types.Config, taskUUID string) error {
	...
	if !keep_tmp {
		...
		{
			nvram := filepath.Join(libvirtImgDir, fmt.Sprintf(nvramImagePattern, taskUUID, "*"))
			matches, err := filepath.Glob(nvram)
			if err != nil {
				return err
			}
			...
			for _, m := range matches {
				err := os.Remove(m)
				log.Debug().Msgf("Deleting nvram image: %s", m)
				if err != nil {
					return err
				}
			}
