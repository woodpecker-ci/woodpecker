...
func newDummyRuntime(t *testing.T, tracer *mockTracer) *Runtime {
	t.Helper()
	engine := dummy.New()
	r := New(
		&backend.Config{},
...
func TestShouldSkipStep(t *testing.T) {
	...
	t.Run("NoErrorOnSuccessTrue", func(t *testing.T) {
		t.Parallel()
		r := newDummyRuntime(t, &mockTracer{})
...
func TestTraceStep(t *testing.T) {
	...
	t.Run("TracerError", func(t *testing.T) {
		...
		tracer := &mockTracer{fn: func(_ *state.State) error { return traceErr }}
		r := newDummyRuntime(t, tracer)
...
// ---------------------------------------------------------------------------
// startStep — uses dummy for success + start/tail failures; mock for logger test
// ---------------------------------------------------------------------------
...
func TestStartStep(t *testing.T) {
	...
	t.Run("WithLogger", func(t *testing.T) {
		t.Parallel()
		// Uses mockEngine so we can verify the logger receives the reader.
		var logCalled int32
		engine := &mockEngine{
			tailStepFn: func(_ context.Context, _ *backend.Step, _ string) (io.ReadCloser, error) {
				return io.NopCloser(strings.NewReader("log line")), nil
			},
		}
		r := New(&backend.Config{}, WithBackend(engine), WithTracer(&mockTracer{}),
...
// ---------------------------------------------------------------------------
// completeStep — uses mockEngine because completeStep needs fine-grained
// control over WaitStep/DestroyStep return values that dummy cannot provide.
// ---------------------------------------------------------------------------
...
func TestCompleteStep(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		t.Parallel()
		r := New(&backend.Config{}, WithBackend(&mockEngine{}), WithTracer(&mockTracer{}))
	...
	t.Run("NonZeroExitCode", func(t *testing.T) {
		t.Parallel()
		engine := &mockEngine{
			waitStepFn: func(_ context.Context, _ *backend.Step, _ string) (*backend.State, error) {
	...
	t.Run("DestroyStepError", func(t *testing.T) {
		t.Parallel()
		engine := &mockEngine{
			destroyStepFn: func(_ context.Context, _ *backend.Step, _ string) error {
...
// ---------------------------------------------------------------------------
// executeStep — uses dummy backend for the full step lifecycle
// ---------------------------------------------------------------------------
...
func TestExecuteStep(t *testing.T) {
	...
	t.Run("SkippedStepTraced", func(t *testing.T) {
		...
		tracer := &mockTracer{}
		r := newDummyRuntime(t, tracer)
		// No error in pipeline + OnSuccess=false → skip
		step := &backend.Step{
	...
	t.Run("BlockingStepSuccess", func(t *testing.T) {
		...
		assert.NoError(t, err)
		calls := tracer.getCalls()
		// Expect 2 traces: started + completed
	...
	t.Run("DetachedStep", func(t *testing.T) {
		...
		assert.NoError(t, err)
		// Wait for the background goroutine to finish and trace.
		assert.Eventually(t, func() bool {
			return len(tracer.getCalls()) >= 2
...
func TestRunBlockingStep(t *testing.T) {
	...
	t.Run("DestroyStepErrorMappedToErrCancel", func(t *testing.T) {
		t.Parallel()
		// Uses mockEngine: dummy cannot make DestroyStep return context.Canceled.
		tracer := &mockTracer{}
		engine := &mockEngine{
			destroyStepFn: func(_ context.Context, _ *backend.Step, _ string) error {
...
func TestRunDetachedStep(t *testing.T) {
	...
	t.Run("ReturnsImmediately", func(t *testing.T) {
		...
		assert.NoError(t, err)
		// Background goroutine should eventually trace completion.
		assert.Eventually(t, func() bool {
			return len(tracer.getCalls()) >= 1
		}, time.Second, 10*time.Millisecond)
	})
