  ...
  >
    <div class="grid grid-cols-[auto,1fr] gap-y-1 items-center">
      ...
      <div class="col-span-2 text-wp-text-100">
        <div v-if="lastPipeline" class="flex gap-x-1 items-center">
      ...
      <div class="col-span-2 text-wp-text-100 text-sm min-h-5 mt-3">
        <div v-if="lastPipeline" class="flex gap-x-4 items-center">
          <div class="flex items-center gap-x-1">
            <span :title="pipelineEventTitle">
              <Icon v-if="lastPipeline.event === 'pull_request'" name="pull-request" size="20" />
              <Icon v-else-if="lastPipeline.event === 'pull_request_closed'" name="pull-request-closed" size="20" />
              <Icon v-else-if="lastPipeline.event === 'deployment'" name="deployment" size="20" />
              <Icon v-else-if="lastPipeline.event === 'tag' || lastPipeline.event === 'release'" name="tag" size="20" />
              <Icon v-else-if="lastPipeline.event === 'cron'" name="push" size="20" />
              <Icon v-else-if="lastPipeline.event === 'manual'" name="manual-pipeline" size="20" />
          ...
          <div class="hidden sm:flex gap-x-1 items-center">
            ...
            <span class="truncate">{{ lastPipeline.commit.slice(0, 10) }}</span>
          </div>
...
const lastPipeline = computed(() => repo.value.last_pipeline_item);
const { since, shortMessage, prettyRef } = usePipeline(lastPipeline);

const { t } = useI18n();

const pipelineEventTitle = computed(() => {
  switch (lastPipeline.value?.event) {
    case 'pull_request':
