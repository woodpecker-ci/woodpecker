<template>
  <div class="md:min-w-xs text-wp-text-100 flex w-full flex-col gap-2 pb-2 md:w-3/12 md:max-w-md">
    <div
      class="bg-wp-background-100 border-wp-background-400 dark:bg-wp-background-200 flex flex-shrink-0 flex-wrap justify-between gap-1 rounded-md border p-4"
    >
    ...
    <div class="relative min-h-0 w-full flex-grow">
      <div class="absolute left-0 right-0 top-0 flex h-full flex-col gap-y-2 md:overflow-y-auto">
        <div
          ...
          :key="workflow.id"
          class="bg-wp-background-100 border-wp-background-400 dark:bg-wp-background-200 rounded-md border p-2 shadow"
        >
