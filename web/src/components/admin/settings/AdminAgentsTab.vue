      ...
      >
        ...
        <span class="ml-auto">
          <span class="hidden md:inline-block space-x-2">
            <Badge v-if="agent.org_id != -1 && agent.repo_id == -1" :label="$t('admin.settings.agents.org.badge')" :value="agent.org_id" />
...
const { doSubmit: saveAgent, isLoading: isSaving } = useAsyncAction(async () => {
  ...
  selectedAgent.value.filters = Object.fromEntries(
    filters.value.map(filter => [filter.key, filter.value])
  );
