...
func (e *EncryptedStore) SecretList(repo *model.Repo, b bool, p *model.ListOptions) ([]*model.Secret, error) {
	results, err := e.store.SecretList(repo, b, p)
	if err != nil {
		return nil, err
	}
...
func (e *EncryptedStore) OrgSecretFind(s int64, s2 string) (*model.Secret, error) {
...
func (e *EncryptedStore) OrgSecretList(s int64, p *model.ListOptions) ([]*model.Secret, error) {
	results, err := e.store.OrgSecretList(s, p)
	if err != nil {
		return nil, err
	}
...
func (e *EncryptedStore) GlobalSecretList(p *model.ListOptions) ([]*model.Secret, error) {
	results, err := e.store.GlobalSecretList(p)
	if err != nil {
		return nil, err
	}
