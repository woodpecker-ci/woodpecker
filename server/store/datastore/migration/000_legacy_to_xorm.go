...
var legacy2Xorm = task{
	...
	fn: func(sess *xorm.Session) error {
		...
		for _, mig := range []string{
			...
			// steps
			"create-table-steps",
			"create-index-steps-build",
			// files
			"create-table-files",
			"create-index-files-builds",
			"create-index-files-steps",
		...
		case schemas.MYSQL:
			for _, exec := range []string{
				...
				"DROP INDEX IF EXISTS step_build_ix ON steps;",
				"DROP INDEX IF EXISTS file_build_ix ON files;",
				"DROP INDEX IF EXISTS file_step_ix  ON files;",
		...
		case schemas.SQLITE, schemas.POSTGRES:
			for _, exec := range []string{
				...
				"DROP INDEX IF EXISTS step_build_ix;",
				"DROP INDEX IF EXISTS file_build_ix;",
				"DROP INDEX IF EXISTS file_step_ix;",
