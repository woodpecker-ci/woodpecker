...
func Create(ctx context.Context, _store store.Store, repo *model.Repo, pipeline *model.Pipeline) (*model.Pipeline, error) {
	...
	if pipeline.Event == model.EventPush || pipeline.IsPullRequest() {
		...
		if len(skipMatch) > 0 {
			...
			log.Info().Str("repo", repo.FullName).Msgf("ignoring pipeline as skip-ci was found in the commit (%s) message '%s'", ref, pipeline.Message)
	...
	if errors.Is(configFetchErr, &forge_types.ErrConfigNotFound{}) {
		log.Info().Str("repo", repo.FullName).Err(configFetchErr).Msgf("cannot find config '%s' in '%s' with user: '%s'", repo.Config, pipeline.Ref, repoUser.Login)
	...
	if pipeline_errors.HasBlockingErrors(parseErr) {
		log.Warn().Str("repo", repo.FullName).Err(parseErr).Msg("failed to parse yaml")
