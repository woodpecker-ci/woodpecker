...
func setupSecretService(store store.Store, endpoint string, client *utils.Client) secret.Service {
	...
	if endpoint != "" {
		return secret.NewCombined(secret.NewDB(store), secret.NewHTTP(endpoint, client))
	}
