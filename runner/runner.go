...
func (r *Runner) Run(ctx context.Context) error {
	...
	err = r.ProcessWork(*work, ctxmeta)
	if err != nil {
		return err
	}
...
func (r *Runner) ProcessWork(work rpc.Pipeline, ctx context.Context) error {
	timeout := time.Hour
	...
	err := r.client.Init(ctx, work.ID, state)
	if err != nil {
		logger.Error().
			Err(err).
	...
	defaultLogger := pipeline.LogFunc(func(proc *backend.Step, rc multipart.Reader) error {
		...
		if serr := r.client.Upload(ctx, work.ID, file); serr != nil {
			loglogger.Error().
				Err(serr).
	...
	defaultTracer := pipeline.TraceFunc(func(state *pipeline.State) error {
		...
		defer func() {
			...
			if uerr := r.client.Update(ctx, work.ID, procState); uerr != nil {
				proclogger.Debug().
					Err(uerr).
	...
	err = r.client.Done(ctx, work.ID, state)
	if err != nil {
