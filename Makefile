...
release-server-xgo: check-xgo ## Create server binaries for release using xgo
	...
	[ "${TARGZ}" -eq "true" ] && tar -cvzf dist/woodpecker-server_$(TARGETOS)_$(TARGETARCH_BUILDX).tar.gz -C dist/server/$(TARGETOS)/$(TARGETARCH_BUILDX) woodpecker-server$(BIN_SUFFIX) || echo "skip tar.gz binary"
...
release-agent: ## Create agent binaries for release
	...
	tar -cvzf dist/woodpecker-agent_windows_amd64.tar.gz -C dist/agent/windows_amd64.exe woodpecker-agent
...
release-cli: ## Create cli binaries for release
	...
	tar -cvzf dist/woodpecker-cli_windows_amd64.tar.gz -C dist/cli/windows_amd64.exe woodpecker-cli
