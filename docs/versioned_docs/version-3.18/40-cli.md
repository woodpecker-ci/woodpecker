...
**--backend-k8s-cluster-domain**="": backend k8s cluster domain, used to build the DNS search entry (default: cluster.local)
...
**--backend-k8s-runtime-class-allow-from-step**: whether to allow using runtime class name from step's backend options (default: false)
...
**--plugins-trusted-clone**="": plugins that are trusted to handle Git credentials in cloning steps (default: "docker.io/woodpeckerci/plugin-git:2.9.3", "docker.io/woodpeckerci/plugin-git", "quay.io/woodpeckerci/plugin-git")
