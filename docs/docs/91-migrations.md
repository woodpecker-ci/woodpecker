...
- Removed build-in environment variables `CI_STEP_FINISHED` and `CI_PIPELINE_FINISHED` as it always was empty durring execution anyway.
- Removed build-in environment variables `CI_PIPELINE_STATUS` and `CI_STEP_STATUS` as it always was `success` anyway.
