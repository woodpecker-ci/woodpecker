...
`ListEditor` renders one read-only row per entry with a delete button, plus an input and an add button for new entries. Entries are not editable in place — they are removed and re-added instead. Use it with `v-model` on a `string[]`:

```vue
<InputField :label="$t('repo.settings.general.netrc_trusted.netrc_trusted')">
  <template #default="{ id }">
    <ListEditor
      :id="id"
...
| ------------- | ----------- | -------------------------------------------------------------------- |
| `modelValue`  | `string[]?` | The current entries. An undefined value is treated as an empty list. |
| `id`          | `string?`   | Id for the new-entry input, so the `InputField` label points at it.  |
| `placeholder` | `string?`   | Placeholder of the new-entry input.                                  |
| `addTitle`    | `string?`   | Tooltip of the add button.                                           |
| `deleteTitle` | `string?`   | Tooltip of the delete buttons.                                       |

A new entry is added by pressing enter in the input or by clicking the add button. Entries are trimmed, and blank or already listed values are ignored — a rejected duplicate stays in the input so it is visible that it was not added. The component never mutates the array it was given, it emits a new one via `update:modelValue`.

If the surrounding form should also save an entry the user typed but never confirmed, call the exposed `commitPendingItem()` on submit:

```vue
<ListEditor ref="imagesEditor" v-model="innerValue.images" />
```
