...
docker buildx build . --platform $PLATFORMS -t $TAG -f docker/Dockerfile.server.multiarch  --push # This will push the image to the registry, use --load to load it only locally (only single arch allowed)
```
