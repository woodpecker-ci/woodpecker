...
Agents are configured by the command line or environment variables.
...
By default, the maximum workflows that are executed in parallel on an agent is 1.
If required, you can add `WOODPECKER_MAX_WORKFLOWS` to increase your parallel processing for an agent.
...
An _agent token_ is a token that is used by only one particular agent.
This unique token is applied to the agent by `WOODPECKER_AGENT_SECRET`.
...
Use a list of key-value pairs like `key=value,second-key=*`.
...
Configures the backend engine to run pipelines on.
